<h1 style="display:flex;"><span>{{ $User->prenom }} {{ $User->nom }}</span><div style="text-align:right;flex-grow:1;"><span style="text-align:center; width: 256px;" class="btn btn-primary" onclick="AjaxFill('{{ route('users.edit',['user' => Auth::id(), 'profil' => true]) }}', '#ModalContainer', false);">Modifier mon profil</span></div></h1>


<h2 style="display:flex;flex-wrap:wrap;">Liste de mes inscriptions</h2>
@if ($AtelierHoraires->count() == 0)
    <br />
    <br />
    <br />
    <br />
    <p style="font-weight:bold; font-size:20px; text-align:center;">Aucun atelier auquel vous êtes inscrit...</p>
    <br />
@else
                            ...
                            <div style="display:table; width:100%; margin-bottom:3.5px; position: relative;">
                                <tr style="display:flex;">
                                    ...
                                    <div class="card-body" style="display:table-cell;background-color:white;flex-grow:1;">
                                        <h5 class="card-title" style="display:inline-block;">{{ substr($Horaire->dateAtelier, -8, 5) }} - {{ $Horaire->salle_local}}</h5>
